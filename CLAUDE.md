...
**MQA detection** (`detect_mqa`, display-level): metadata scan for "mqa", then signal-level — MQA carries a control stream in `(L XOR R)` at bit position (depth−16) starting with the 36-bit sync word `0xbe0498c88` (reverse-engineered, MQA_identifier project); numpy rolling-window match over the first 4 s catches tag-stripped MQA. Caveat + Source Integrity row; the folded payload itself is unverifiable by PCM analysis.
...
After any threshold change, re-run `python test_dsp.py`, regenerate the REAL-MUSIC battle fixtures with `python testdata/gen_battle.py` (gitignored; builds mp3 128/320, aac 128/256, upsample 44.1→96, downsample-up 22.05→44.1, a 30 s spliced MP3 span, padded 24-bit, and a direct .mp3 — all from `testdata/FLAC_Redbook.flac`), run the tool on them plus the real-world controls (`MFSLUDCD_redbook`, `Vinyl24-*`, `web24-96`, `vinyl DSD128*`, `mqa.flac`, `opus.m4a`, `test_lc.m4a`, `sacddsd64.dff`) and check with `python testdata/summarize.py <json>`: walled fakes ≥86, upsample/spliced ≥45–55 SUSPICIOUS, every genuine control GENUINE/≤20. Pink noise fixtures pass detectors that real music defeats (dark passages hide cliffs) — always validate on the real-music set. Known miss: AAC ≥256 kbps full-bandwidth transcodes (no wall; MDCT block-grid analysis is the open research item).
